...
func (n *layerNode[K]) search(
	...
	for candidates.Len() > 0 {
		...
		for _, neighborID := range neighborKeys {
			...
			// IDX-7: a corrupt/truncated graph import can leave a nil neighbor
			// reference; skip it rather than nil-deref neighbor.Value below.
...
func (n *layerNode[K]) replenish(m int, distance DistanceFunc) {
	if len(n.neighbors) >= m {
		return
	}
	...
	for _, neighbor := range n.neighbors {
		for key, candidate := range neighbor.neighbors {
			...
			n.addNeighbor(candidate, m, distance)
			if len(n.neighbors) >= m {
				return
			}
		}
...
// to neighbors.
func (n *layerNode[K]) isolate(m int, distance DistanceFunc) {
	for _, neighbor := range n.neighbors {
		delete(neighbor.neighbors, n.Key)
		neighbor.replenish(m, distance)
	}
}
...
func (l *layer[K]) entry() *layerNode[K] {
	...
	// IDX-10: return a deterministic entry (smallest key) so graph build and search
	// are reproducible. It doesn't matter which node is returned, only that it's stable.
	var minKey K
	first := true
	for k := range l.nodes {
		if first || k < minKey {
...
func (g *Graph[K]) Add(nodes ...Node[K]) {
	for _, node := range nodes {
		...
		preLen := g.Len()
		// IDX-4: a same-key Add is a replace (the in-loop Delete+reinsert keeps the
		// base-layer count constant), so the post-condition must expect preLen, not
		// preLen+1. Reading a nil map is safe and returns (zero,false).
		_, isReplace := g.layers[0].nodes[key]
		...
		for i := len(g.layers) - 1; i >= 0; i-- {
			...
			if insertLevel >= i {
				// HNW-2: a same-key replace must be scoped to THIS layer. The old
				// g.Delete(key) removed the key from EVERY layer and could leave the
				// elevator pointing at the now-deleted old copy, nil-dereferencing on
				// the next-lower layer. Isolate + remove the old node from this layer
				// only; lower layers still hold the old key for the elevator to land on
				// until their own iteration replaces them.
				if old, ok := layer.nodes[key]; ok {
					old.isolate(g.M, g.Distance)
					delete(layer.nodes, key)
				}
				// Insert the new node into the layer.
				layer.nodes[key] = newNode
				for _, node := range neighborhood {
					// Never link the new node to its own stale copy (same key).
...
func (h *Graph[K]) Search(near Vector, k int) []Node[K] {
	...
	for layer := len(h.layers) - 1; layer >= 0; layer-- {
		...
		if elevator != nil {
			if e := h.layers[layer].nodes[*elevator]; e != nil {
				searchPoint = e
			}
		}

		// HNW-4: defend against a nil entry (an empty or inconsistent layer) rather
		// than dereferencing it. Descend to the next-lower layer; if even the base
		// layer is empty, fall through and return no results instead of panicking.
		if searchPoint == nil {
			continue
		}
...
func (h *Graph[K]) Delete(key K) bool {
	...
	// HNW-4: trim now-empty upper layers so a subsequent Search never lands on a
	// nil entry(). By HNSW's nested-layer invariant a node at level L lives in
	// layers 0..L, so an empty layer implies every layer above it is empty too —
	// trimming from the top is safe. If the graph is fully emptied, h.layers
	// collapses to nil and Len()/Search handle the zero-layer case.
