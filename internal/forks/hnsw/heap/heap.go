...
// maxIndex returns the index of the maximum element (per Less). This heap is a
// MIN-heap, so the maximum is NOT data[len-1] (an arbitrary leaf) — it must be
// located. Cheap here: callers use heaps bounded by k / efSearch.
...
// PopLast removes and returns the maximum element (the "worst" / farthest in the
// HNSW search heaps).
...
// Max returns the maximum element in the heap (per Less).
func (h *Heap[T]) Max() T {
